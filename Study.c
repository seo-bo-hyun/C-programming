...
practice()
{
	...
	int addmoney, nowmoney, nowmoney1, nowmoney2, nowmoney3, addmoney1, addmoney2;
	int tteok, udong, snack;
	int tteokprice, udongprice, snackprice;
	int tteokcount, udongcount, snackcount;
	int totalprice;
	int newtteok, newudong, newsnack;
	...
	printf("떡볶이의 가격은 %d원 입니다 \n", tteokprice);

	nowmoney1 = nowmoney - tteokprice;

	printf("현재 남은 용돈은 %d원 입니다\n", nowmoney1);

	if (nowmoney1 < 0)
	{
		printf("떡볶이를 구매하기 위한 용돈이 부족합니다\n");
		printf("추가로 받을 용돈을 입력하세요 : \n");
		scanf("%d", &addmoney);

		nowmoney2 = nowmoney1 + addmoney;

		printf("%d원을 추가 받아서 현재 용돈은 %d원입니다 \n", addmoney, nowmoney2);

		printf("떡볶이 몇 개를 구매하시겠습니까? \n");
		scanf("%d", &newtteok);

		printf("떡볶이 %d개를 구매하셨습니다\n", newtteok);

		tteokprice = tteok * newtteok;

		printf("다시 구매한 떡볶이의 가격은 %d원 입니다\n", tteokprice);

		nowmoney3 = nowmoney2 - tteokprice;

		printf("현재 남은 용돈은 %d원 입니다\n", nowmoney3);
	}

	printf("떡볶이의 가격은 %d원 이었고 현재 남은 금액은 %d원 입니다 \n", tteokprice, nowmoney1);
